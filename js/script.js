...
//随机文字烟花内容
//const randomWords = ["CHÚC MỪNG NĂM MỚI | Vân ơi"];
//const wordDotsMap = {};
...
document.addEventListener("DOMContentLoaded", function () {
	...
	// 设置背景图像和背景大小
	// 在这里输入图片路径
	//canvasContainer.style.backgroundImage = "url('picture/Capture.JPG')";
...
// 隨機取得一段文字（完整字串，例如 "CHÚC MỪNG NĂM MỚI | Vân ơi"）
function randomWord() {
...
// 如果你需要在煙花中分別顯示「主標題」和「副標題」：
function getRandomTitleAndSubtitle() {
    const fullText = randomWord();
    const parts = fullText.split(" | ");  // 注意這裡有個空格！
    return {
        full: fullText,
        title: parts[0] || "",
        subtitle: parts[1] || ""
